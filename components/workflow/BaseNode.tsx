...
import { NodeCategory, WorkflowNode } from '@/types/types';
...
export const BaseNode: React.FC<BaseNodeProps> = ({ data, isConnectable }) => {
  ...
  return (
    <div 
      className="min-w-50 rounded-lg bg-[rgb(25,25,25)] border border-[rgb(51,51,51)] p-3 shadow-lg cursor-pointer hover:border-[rgb(100,100,100)] transition-colors"
      onClick={(e) => {
        e.stopPropagation();
        setSelectedNodeId(data.label || '');
      }}
    >
      {/* Input Handles */}
        ...
        {data.definition?.inputs?.map((input) => (
          <div key={`input-field-${input.id}`} className="flex items-center gap-2">
            <div 
              ...
              style={{ backgroundColor: getPortColor(input.id) }}
            />
