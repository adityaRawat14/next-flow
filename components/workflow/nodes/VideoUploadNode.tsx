...
export function VideoUploadNode({ data, selected }: any) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      alert("Please upload a valid video (mp4, mov, webm, m4v)");
      return;
    }
  ...
  return (
    <div
      ...
      } ${data.isExecuting ? "animate-pulse bg-purple-500/10" : ""}`}
    >
      {/* Header */}
        ...
        <div className="space-y-3">
          {videoUrl ? (
            <div className="relative">
              <video
                src={videoUrl}
                className="w-full h-40 bg-[#0a0a0a] rounded object-cover"
                controls
              />
              <button
                onClick={() => {
            ...
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-[#444] rounded-lg cursor-pointer hover:border-purple-500 hover:bg-purple-500/5 transition">
